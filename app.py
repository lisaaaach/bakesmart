...
def build_selected_preferences(selected_allergies=None, selected_diets=None, selected_nutrition_goals=None):
    """
    Combines all selected customization preferences into a single clean list.
    """
    selected_allergies = selected_allergies or []
    selected_diets = selected_diets or []
    selected_nutrition_goals = selected_nutrition_goals or []
    return selected_allergies + selected_diets + selected_nutrition_goals


def summarize_suggestion_reason(item, selected_allergies=None, selected_diets=None, selected_nutrition_goals=None):
    """
    Creates a short user-facing explanation for why an ingredient appears in the customization plan.
    """
    reasons = []

    related_info = item.get("related_info", []) if isinstance(item, dict) else []
    if related_info:
        reasons.append("matched " + ", ".join(related_info[:3]))
    ...
    if selected_allergies:
        reasons.append("related to your allergy selections")
    if selected_diets:
        reasons.append("related to your diet preferences")
    if selected_nutrition_goals:
        reasons.append("supports your nutrition goals")

    reasons = list(dict.fromkeys([r for r in reasons if r]))

    if not reasons:
        return "This ingredient matched the selected recipe and substitution database."

    return "; ".join(reasons[:2]).capitalize() + "."


def merge_llm_suggestions_with_plan(combined_substitutions, llm_substitutions):
    """
    Attaches LLM suggestions to the closest matching ingredient card when possible.
    Any unmatched LLM suggestions are returned separately.
    """
    if not llm_substitutions:
        return combined_substitutions, []

    combined_substitutions = combined_substitutions or []
    unmatched = []

    for card in combined_substitutions:
        card["ai_substitutes"] = []

    for llm_item in llm_substitutions:
        original = normalize_text(llm_item.get("original_ingredient"))
        matched = False

        if original:
            for card in combined_substitutions:
                ingredient_norm = normalize_text(card.get("recipe_ingredient"))
                if ingredient_norm and (original in ingredient_norm or ingredient_norm in original):
                    card.setdefault("ai_substitutes", []).append(llm_item)
                    matched = True
                    break

        if not matched:
            unmatched.append(llm_item)

    return combined_substitutions, unmatched

# ============================================================
# MACHINE LEARNING RECOMMENDATION
# ============================================================

def get_ml_recipe_recommendation(conn, recipe_id, source_table=None, recipe_name=None, top_n=3):
    try:
        df_ml_all = pd.read_sql(f"""
            SELECT id, recipe_id, source_table, recipe_name, cluster, cluster_name, pca_1, pca_2
            FROM {ML_TABLE}
        """, conn)
    except Exception:
        return {
    ...
    df_ml_all["recipe_id_str"] = df_ml_all["recipe_id"].astype(str)
    df_ml_all["source_table_str"] = df_ml_all["source_table"].astype(str)
    ...
    # 1. Best match: recipe_id + source_table
    selected_ml = df_ml_all[
        (df_ml_all["recipe_id_str"] == recipe_id_str) &
        (df_ml_all["source_table_str"] == source_table_str)
    ]

    # 2. Fallback: recipe_id only
    if selected_ml.empty:
        selected_ml = df_ml_all[df_ml_all["recipe_id_str"] == recipe_id_str]

    # 3. Fallback: exact recipe name
    if selected_ml.empty and recipe_name_norm:
        selected_ml = df_ml_all[df_ml_all["recipe_name_norm"] == recipe_name_norm]

    # 4. Fallback: partial recipe name
    if selected_ml.empty and recipe_name_norm:
        selected_ml = df_ml_all[
...
if "selected_recipe" in st.session_state:
    ...
    # ========================================================
    # UNIFIED CUSTOMIZATION PLAN
    # ========================================================

    selected_preferences = build_selected_preferences(
        selected_allergies=selected_allergies,
        selected_diets=selected_diets,
        selected_nutrition_goals=selected_nutrition_goals
    )

    preference_signature = json.dumps({
        "recipe_id": str(selected_id),
        "allergies": selected_allergies,
        "diets": selected_diets,
        "nutrition_goals": selected_nutrition_goals
    }, sort_keys=True)

    if st.session_state.get("customization_signature") != preference_signature:
        st.session_state["customization_signature"] = preference_signature
        st.session_state.pop("llm_substitutions", None)
    ...
    ml_result = get_ml_recipe_recommendation(
        conn,
        selected_id,
        source_table=st.session_state.get("selected_recipe_source"),
    ...
    section_card(
        ...
        "A concise plan that combines allergy matching, database/community substitution examples, optional AI suggestions, and similar recipe recommendations."
    )

    if selected_preferences:
        st.write("**Your selected preferences:**")
        render_badges(selected_preferences)
    else:
    ...
    st.subheader("AI-Enhanced Suggestions")

    has_customization = bool(selected_preferences)
    ...
    if has_customization:
        ...
        if "llm_substitutions" in st.session_state:
            ...
            if llm_substitutions:
                combined_with_ai, unmatched_ai = merge_llm_suggestions_with_plan(
                ...
                with st.container(border=True):
                    ...
                    with text_col:
                        st.markdown(
                            f"### AI Customization for {display_value(selected_recipe.get('recipe_name'), 'This Recipe')}"
                        )
                        st.write("**Based on:** " + ", ".join(selected_preferences))
                        st.write(
                            "These suggestions are generated by the LLM and should be reviewed with baking judgment, "
                            "especially for texture-sensitive desserts."
                        )

                shown_ai_count = 0

                for item in combined_with_ai:
                    for ai_item in item.get("ai_substitutes", []):
                        shown_ai_count += 1
                        original = display_value(ai_item.get("original_ingredient"), "Original ingredient")
                        substitute = display_value(ai_item.get("substitute_ingredient"), "Suggested substitute")
                        amount = display_value(ai_item.get("recommended_amount"))
                        reason = display_value(ai_item.get("reason"))

                        with st.container(border=True):
                            st.markdown(f"#### AI Suggestion {shown_ai_count}: {original} → {substitute}")
                            st.write(f"**Recommended Amount:** {amount}")
                            st.write(f"**Reason:** {reason}")

                for ai_item in unmatched_ai:
                    shown_ai_count += 1
                    original = display_value(ai_item.get("original_ingredient"), "Original ingredient")
                    substitute = display_value(ai_item.get("substitute_ingredient"), "Suggested substitute")
                    amount = display_value(ai_item.get("recommended_amount"))
                ...
                st.info("No AI substitution recommendations were generated.")
    else:
        st.info("Choose at least one allergy, diet preference, or nutrition goal to generate AI suggestions.")

    # -----------------------------
    # Similar recipes from ML
    # -----------------------------
    
    with st.expander("Similar Recipes You May Like", expanded=False):
    ...
    with st.expander("Advanced matching details"):
        st.write("These tables show the backend matching results used to build the customization plan.")

        st.write("**All allergy/database matches for this recipe:**")
        if all_allergy_hits is not None and not all_allergy_hits.empty:
            st.dataframe(all_allergy_hits, use_container_width=True)
        else:
            st.write("No allergy/database matches found for this recipe.")

        st.write("**Filtered allergy matches based on selected allergies:**")
        if filtered_allergy_hits is not None and not filtered_allergy_hits.empty:
            st.dataframe(filtered_allergy_hits, use_container_width=True)
        else:
            st.write("No filtered allergy matches for the selected allergies.")

        st.write("**Database substitution rows used:**")
        if database_substitutions is not None and not database_substitutions.empty:
            st.dataframe(database_substitutions, use_container_width=True)
        else:
            st.write("No database substitution rows matched the selected preferences.")

        st.write("**Community substitution rows used:**")
        if community_substitutions is not None and not community_substitutions.empty:
            st.dataframe(community_substitutions, use_container_width=True)
        else:
            st.write("No community substitution rows matched this recipe.")
